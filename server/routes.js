...
var mailgun = require("mailgun-js")({
	...
	domain:
		process.env.NODE_ENV === "development"
			? config.mailgun.api_sandbox_domain
			: config.mailgun.api_domain,
});
		...
		req.getValidationResult().then(function (result) {
			if (!result.isEmpty()) {
				...
				return res.json({
					...
					errors: errors,
					message:
						"There was a problem sending your request, please contact us by phone at 423-202-3228",
				});
			} else {
				...
				mailgun.messages().send(mailgun_data, function (error, body) {
					if (error) {
						...
						return res.json({
							...
							errors: [],
							message:
								"Thank you! Your private instruction request has been received, we will be in touch with you shortly!",
						});
					}
				});
			}
	...
	app.post("/equipment-request", function (req, res) {
		...
		req.getValidationResult().then(function (result) {
			if (!result.isEmpty()) {
				...
				// validation ok, send the email
				var message =
					"The following information was just submitted for equipment: \n\n" +
					"Player Name: " +
					req.body.firstName +
					" " +
					req.body.lastName +
				...
				mailgun.messages().send(mailgun_data, function (error, body) {
					if (error) {
						return res.json({
							...
							errors: ["error"],
							message:
								"There was a problem sending your equipment request, please contact us by phone at 423-202-3228",
						});
					} else {
						return res.json({
							status: 200,
							errors: [],
							message:
								"Thank you! Your equipment request has been received, Wilson will be in touch with you within 24 hours!",
						});
					}
				});
			}
		...
		app.locals.leagueData = [
			{
				...
				link:
					"https://hittraxstatscenter.com/widget/tournament_widget.php?TId=1&TUId=552&SId=552&UId=552",
			},
			{
				name: "Virtual Hitting Leagues - Middle School Baseball",
				date: "January 8th - February 5th, 2018",
				link:
					"https://hittraxstatscenter.com/widget/tournament_widget.php?TId=2&TUId=552&SId=552&UId=552",
			},
			{
				name: "Virtual Hitting Leagues - 10U & 12U Baseball",
				date: "January 27th - February 24th, 2018",
				link:
					"https://hittraxstatscenter.com/widget/tournament_widget.php?TId=3&TUId=552&SId=552&UId=552",
			},
			{
				name: "Virtual Hitting Leagues - Men's League 21 & Up",
				date: "March 4 - April 1, 2018",
				link:
					"https://hittraxstatscenter.com/widget/tournament_widget.php?TId=4&TUId=552&SId=552&UId=552",
			},
		];
